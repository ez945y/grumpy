...
一個小型知識庫，用來放專案從來不會寫下來的那些事。

兩個檔案，純 Python，不用安裝任何東西。筆記就是純文字檔，任何編輯器都打得開。

## 為什麼

當 AI 助理在一個大型程式庫上工作時，它會一次又一次地把同樣的事情重新弄懂：
哪個部分管什麼、為什麼在乾淨的副本上建置會失敗、當初對這件事做了什麼決定又
是為什麼。這些東西沒有一樣能延續到下一次對話。

一般的做法是開一個越寫越長的筆記檔。這在它還短的時候有效，一旦長到讀完它的
成本超過答案本身，就失效了。

grumpy 讓自己保持小：

- **一則筆記回答一個問題。** 筆記限制在大約半頁，所以搜尋結果就是答案，
  不是一份還要讀完的東西。
- **比較大的東西另外放。** 描述各部分如何組合的地圖放在 `docs/`，一次讀
  一個段落。搜尋只給你它的目錄，不是全文。
- **標記為已解決的就不再出現**，而且它會告訴你藏了幾筆。
- **筆記之間雙向互指**，所以找到一則就能連到整件事的其餘部分。

它叫 grumpy 是因為它會拒絕寫入。如果一則筆記重複了你已經有的、寫得太長、
或者用了臨時發明的標籤，它會停下來並告訴你它找到了什麼。確定要寫就加 `-f`。
